...
/**
 * Tests the LimitedDrawKlondike class.
 */
public class LimitedKlondikeTests {
  LimitedDrawKlondike model;
  List<Card> deck;
  KlondikeTextualView textView;
  LimitedDrawKlondike noDrawCards;
  List<Card> threeCardDeck;

  @Before
  public void init() {
    model = new LimitedDrawKlondike(0);
    deck = model.getDeck();
    textView = new KlondikeTextualView(model);

    noDrawCards = new LimitedDrawKlondike(0);
  ...
  @Test
  public void testGameWithInvalidRedrawAttempts() {
    this.init();
    Assert.assertThrows("Cannot have negative redraw attempts",
        IllegalArgumentException.class, () ->  new LimitedDrawKlondike(-1));
  }
  ...
  @Test
  public void testMoveDrawAfterDiscardingCards() {
    this.init();
    model = new LimitedDrawKlondike(1);
    ...
    model.moveDraw(1);
    Assert.assertEquals("move was successful", model.getPileHeight(1), 3);
    //discard the rest
    for (int i = 0; i < 4; i++) {
      model.discardDraw();
    }
    Assert.assertThrows("cannot move draw pile card if draw pile is empty",
  ...
  public void testMoveDrawToFoundation() {
    ...
    Assert.assertEquals("Card should have been moved", model.getScore(), 1);

    for (int i = 0; i < 23; i++) {
      model.discardDraw();
    }
    model.moveDrawToFoundation(0);
    Assert.assertEquals("Card should have been moved", model.getScore(), 2);
    for (int i = 0; i < 11; i++) {
      model.discardDraw();
    }

    Assert.assertThrows("Card cannot be moved if draw pile is empty",
        IllegalStateException.class, () -> model.moveDrawToFoundation(0));
  }
  ...
  public void testDiscardDraw() {
    ...
    model.startGame(model.getDeck(), false, 7, 24);
    ...
    Assert.assertEquals("No cards should have been discarded",
        model.getDrawCards().size(), 24);

    for (int i = 0; i < 24; i++) {
      model.discardDraw();
    }
    Assert.assertEquals("All cards should have been discarded",
        model.getDrawCards().size(), 0);
  }
  ...
  public void testGetDrawCards() {
    ...
    Assert.assertEquals("Draw pile should now be empty", model.getDrawCards().size(), 0);
  }
